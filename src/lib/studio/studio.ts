...
import AnimationManager from '@/lib/studio/v2/AnimationManager';
...
class Studio extends EditorEventEmitter<EditorEventMap> {
  ...
  private updateCurrentFrame(frame: number) {
    ...
    this.emit('timeUpdated', frameToTimeMillis(this.currentFrame, this.settings.fps) / 1000);
  }
  ...
  public reset() {
    gsap.to(this.mainContainer.scale, {
      x: this.baseScale,
      y: this.baseScale,
      duration: 0.3,
      ease: 'power3.out',
